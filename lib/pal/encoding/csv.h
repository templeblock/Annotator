...
public:
	...
	// [starts[1], starts[2])
	//        ...  starts[N])
	// A terminal is always added to starts, so that the number of cells is starts.size() - 1.
	// An empty line produces a single empty cell, so starts.size() is never less than 2 (1 for the empty cell + 1 for the terminal).
