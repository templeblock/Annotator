...
std::wstring IMQS_PAL_API ConvertUTF8ToWide(const std::string& src);
std::string IMQS_PAL_API ConvertWideToUTF8(const std::wstring& src);
std::string IMQS_PAL_API ConvertUTF16ToUTF8(const uint16_t* src, size_t srcLen);
...
**/
bool IMQS_PAL_API ConvertWideToUTF8(const wchar_t* src, size_t srcLen, char* dst, size_t& dstLen, bool relaxNullTerminator = false);

/// Analogue of ConvertWideToUTF8
bool IMQS_PAL_API ConvertUTF8ToWide(const char* src, size_t srcLen, wchar_t* dst, size_t& dstLen, bool relaxNullTerminator = false);

bool IMQS_PAL_API ConvertWideToUTF16(const wchar_t* src, size_t srcLen, uint16_t* dst, size_t& dstLen, bool relaxNullTerminator = false);
bool IMQS_PAL_API ConvertUTF16ToWide(const uint16_t* src, size_t srcLen, wchar_t* dst, size_t& dstLen, bool relaxNullTerminator = false);
...
// Returns true if all bytes in the string are between 0 and 127, inclusive
inline bool IsLowAscii(const char* s, size_t len) {
...
// Returns true if all bytes in the string are between 0 and 127, inclusive
inline bool IsLowAscii(const char* s) {
...
ConversionResult IMQS_PAL_API ConvertUTF8toUTF16(const UTF8** sourceStart, const UTF8* sourceEnd, UTF16** targetStart, UTF16* targetEnd, ConversionFlags flags);
ConversionResult IMQS_PAL_API ConvertUTF16toUTF8(const UTF16** sourceStart, const UTF16* sourceEnd, UTF8** targetStart, UTF8* targetEnd, ConversionFlags flags);
ConversionResult IMQS_PAL_API ConvertUTF8toUTF32(const UTF8** sourceStart, const UTF8* sourceEnd, UTF32** targetStart, UTF32* targetEnd, ConversionFlags flags);
ConversionResult IMQS_PAL_API ConvertUTF32toUTF8(const UTF32** sourceStart, const UTF32* sourceEnd, UTF8** targetStart, UTF8* targetEnd, ConversionFlags flags);
ConversionResult IMQS_PAL_API ConvertUTF16toUTF32(const UTF16** sourceStart, const UTF16* sourceEnd, UTF32** targetStart, UTF32* targetEnd, ConversionFlags flags);
ConversionResult IMQS_PAL_API ConvertUTF32toUTF16(const UTF32** sourceStart, const UTF32* sourceEnd, UTF16** targetStart, UTF16* targetEnd, ConversionFlags flags);

bool IMQS_PAL_API IsLegalUTF8Sequence(const UTF8* source, const UTF8* sourceEnd);
}
}
}
