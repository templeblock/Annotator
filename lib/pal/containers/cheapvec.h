...
protected:
	...
	void resizeto(size_t newcap, bool initmem) {
		if (newcap == 0)
			return;
		T* newdata = (T*) imqs_realloc_or_die(data, newcap * sizeof(T));
		data       = newdata;
...
namespace std {
template <typename T>
inline void swap(imqs::cheapvec<T>& a, imqs::cheapvec<T>& b) {
	char tmp[sizeof(imqs::cheapvec<T>)];
