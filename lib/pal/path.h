...
IMQS_PAL_API bool IsAbsolute(const std::string& path);               // Returns true if the path starts with a forward slash, or X:
...
IMQS_PAL_API std::string MakeSlashesNative(const std::string& path); // If Unix, convert all backslashes to forward slashes. On Windows, do the opposite. This is not always a reliable tool, but in some cases it works.
...
// Join two or more paths together with slashes. However, if any components contains "..", then return the first element of 'parts'
