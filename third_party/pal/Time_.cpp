...
Time::Time(int year, Month month, int day, int hour, int minute, int second, int nsec) {
	...
	if (IsLeapYear(year) && month >= Month::March)
		d++; // February 29
...
void Time::DateComponents(int& year, Month& month, int& day, int& yday) const {
	...
	month = (Month)((int) month + 1); // because January is 1
	day   = day - begin + 1;
}
...
Error Time::Parse8601(const char* str, size_t len) {
	...
	*this     = Time(year, (Month) month, day, h, m, static_cast<int>(intpart), static_cast<int>(round(fractpart * 1000000000)));
